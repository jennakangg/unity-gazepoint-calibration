...
public class GazepointClient : MonoBehaviour
{
    ...
    [SerializeField] private int _bufferSize = 4096*2;   /* Increased the buffer size */
    ...
    [SerializeField] private bool _enableReadCounter = true;
    [SerializeField] private bool _enableReadCursor = true;
    [SerializeField] private bool _enableReadLeftEye = true;
    [SerializeField] private bool _enableReadRightEye = true;
    [SerializeField] private bool _enableReadFixedPog = true;
    [SerializeField] private bool _enableReadLeftPog = true;
    [SerializeField] private bool _enableReadRightPog = true;
    [SerializeField] private bool _enableReadBestPog = true;
    [SerializeField] private bool _enableReadLeftPupil = true;
    [SerializeField] private bool _enableReadRightPupil = true;
    [SerializeField] private bool _enableReadTime = true;
    [SerializeField] private bool _enableReadTimeTick = true;
    [SerializeField] private bool _enableReadUserData = true;
    ...
    public class GazeData
    {
        ...
        public float LeftEyeZ { get; set; }
        public float LeftEyePupilDiameter { get; set; }
        public bool LeftEyePupilValid { get; set; }
        public float RightEyeX { get; set; }
        public float RightEyeY { get; set; }
        public float RightEyeZ { get; set; }
        public float RightEyePupilDiameter { get; set; }
        public bool RightEyePupilValid { get; set; }
        public float FixedPogX { get; set; }
        public float FixedPogY { get; set; }
        public float FixedPogStart { get; set; }
        public float FixedPogDuration { get; set; }
        public int FixedPogId { get; set; }
        public bool FixedPogValid { get; set; }
        public float LeftPogX { get; set; }
        public float LeftPogY { get; set; }
        public bool LeftPogValid { get; set; }
        public float RightPogX { get; set; }
        public float RightPogY { get; set; }
        public bool RightPogValid { get; set; }
        public float BestPogX { get; set; }
        public float BestPogY { get; set; }
        public bool BestPogValid { get; set; }
        public float LeftPupilX { get; set; }
        public float LeftPupilY { get; set; }
        public float LeftPupilDiameter { get; set; }
        public float LeftPupilScale { get; set; }
        public bool LeftPupilValid { get; set; }
        public float RightPupilX { get; set; }
        public float RightPupilY { get; set; }
        public float RightPupilDiameter { get; set; }
        public float RightPupilScale { get; set; }
        public bool RightPupilValid { get; set; }
    ...
    private List<GazeData> _gazeValues = new List<GazeData>();
    public List<GazeData> GazeValues { get { return _gazeValues; } private set { _gazeValues = value; } }
    ...
    */
    ...
    /*
        Accumuluate all frames in between 
    */
    private void Update()
    {
        byte[] buffer = new byte[_bufferSize];

        // Clear previous gaze values
        _gazeValues.Clear();
        ...
        if (_dataStream != null)
        {
            while (_dataStream.DataAvailable)
            {
                ...
                _dataIn += Encoding.UTF8.GetString(buffer, 0, bytesRead);
            }

            // Process multiple records from the data stream
            MatchCollection matches = Regex.Matches(_dataIn, RECORD_REGEX);
            foreach (Match match in matches)
            {
                _dataIn = match.Value;
                _gazeValues.Add(ParseGazeData());  // Store parsed gaze data in the list
            }

        }
        else
        {
            // Debug.LogWarning("myObject is null!"); // Optional for debugging
        }


        // Clear _dataIn buffer after processing
        _dataIn = "";
    }
    ...
    */
    ...
    /**
        ...
        values.
    */
    private GazeData ParseGazeData()
    {
        GazeData newGazeData = new GazeData();

        if (_enableReadCounter)
        {
            newGazeData.Counter = ParseInt(COUNTER_ID);
        }

        if (_enableReadCursor)
        {
            newGazeData.CursorX = ParseFloat(CURSOR_X_ID);
            newGazeData.CursorY = ParseFloat(CURSOR_Y_ID);
            newGazeData.CursorState = ParseInt(CURSOR_STATE_ID);
        }

        if (_enableReadLeftEye)
        {
            newGazeData.LeftEyeX = ParseFloat(LEFT_EYE_X_ID);
            newGazeData.LeftEyeY = ParseFloat(LEFT_EYE_Y_ID);
            newGazeData.LeftEyeZ = ParseFloat(LEFT_EYE_Z_ID);
            newGazeData.LeftEyePupilDiameter = ParseFloat(LEFT_EYE_PUPIL_DIAMETER_ID);
            newGazeData.LeftEyePupilValid = ParseBool(LEFT_EYE_PUPIL_VALID_ID);
        }

        if (_enableReadRightEye)
        {
            newGazeData.RightEyeX = ParseFloat(RIGHT_EYE_X_ID);
            newGazeData.RightEyeY = ParseFloat(RIGHT_EYE_Y_ID);
            newGazeData.RightEyeZ = ParseFloat(RIGHT_EYE_Z_ID);
            newGazeData.RightEyePupilDiameter = ParseFloat(RIGHT_EYE_PUPIL_DIAMETER_ID);
            newGazeData.RightEyePupilValid = ParseBool(RIGHT_EYE_PUPIL_VALID_ID);
        }

        if (_enableReadFixedPog)
        {
            newGazeData.FixedPogX = ParseFloat(FIXED_POG_X_ID);
            newGazeData.FixedPogY = ParseFloat(FIXED_POG_Y_ID);
            newGazeData.FixedPogStart = ParseFloat(FIXED_POG_START_ID);
            newGazeData.FixedPogDuration = ParseFloat(FIXED_POG_DURATION_ID);
            newGazeData.FixedPogId = ParseInt(FIXED_POG_ID_ID);
            newGazeData.FixedPogValid = ParseBool(FIXED_POG_VALID_ID);
        }

        if (_enableReadLeftPog)
        {
            newGazeData.LeftPogX = ParseFloat(LEFT_POG_X_ID);
            newGazeData.LeftPogY = ParseFloat(LEFT_POG_Y_ID);
            newGazeData.LeftPogValid = ParseBool(LEFT_POG_VALID_ID);
        }

        if (_enableReadRightPog)
        {
            newGazeData.RightPogX = ParseFloat(RIGHT_POG_X_ID);
            newGazeData.RightPogY = ParseFloat(RIGHT_POG_Y_ID);
            newGazeData.RightPogValid = ParseBool(RIGHT_POG_VALID_ID);
        }

        if (_enableReadBestPog)
        {
            newGazeData.BestPogX = ParseFloat(BEST_POG_X_ID);
            newGazeData.BestPogY = ParseFloat(BEST_POG_Y_ID);
            newGazeData.BestPogValid = ParseBool(BEST_POG_VALID_ID);
        }

        if (_enableReadLeftPupil)
        {
            newGazeData.LeftPupilX = ParseFloat(LEFT_PUPIL_X_ID);
            newGazeData.LeftPupilY = ParseFloat(LEFT_PUPIL_Y_ID);
            newGazeData.LeftPupilDiameter = ParseFloat(LEFT_PUPIL_DIAMETER_ID);
            newGazeData.LeftPupilScale = ParseFloat(LEFT_PUPIL_SCALE_ID);
            newGazeData.LeftPupilValid = ParseBool(LEFT_PUPIL_VALID_ID);
        }

        if (_enableReadRightPupil)
        {
            newGazeData.RightPupilX = ParseFloat(RIGHT_PUPIL_X_ID);
            newGazeData.RightPupilY = ParseFloat(RIGHT_PUPIL_Y_ID);
            newGazeData.RightPupilDiameter = ParseFloat(RIGHT_PUPIL_DIAMETER_ID);
            newGazeData.RightPupilScale = ParseFloat(RIGHT_PUPIL_SCALE_ID);
            newGazeData.RightPupilValid = ParseBool(RIGHT_PUPIL_VALID_ID);
        }

        if (_enableReadTime)
        {
            newGazeData.Time = ParseFloat(TIME_ID);
        }

        if (_enableReadTimeTick)
        {
            newGazeData.TimeTick = ParseUlong(TIME_TICK_ID);
        }

        if (_enableReadUserData)
        {
            newGazeData.UserData = ParseString(USER_DATA_ID);
        }
        return newGazeData;
    }
